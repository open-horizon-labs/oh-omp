...
export async function clone(url: string, targetDir: string, options: CloneOptions = {}): Promise<void> {
	...
	// branch, so a later `git checkout <sha>` for a non-tip commit fails with
	// "reference is not a tree". When the caller pins a SHA, fall back to a full
	// clone so the requested object is present in the local store.
