...
- Live provider smoke (Gate 5 / final acceptance obligation):
  - RUN AND PASSED (orchestrator-run, 2026-07-05T19:46:24Z UTC, at `208fde5d8` with a clean tree): `SUCCESSOR_LIVE_PROVIDER_SMOKE=1 cargo test -p successor-kernel --test slice0_end_to_end live_smoke_against_the_real_anthropic_messages_api_produces_a_replayable_terminal_frame -- --ignored --exact` with a real `ANTHROPIC_API_KEY` present in the environment (value never displayed). Observed: `test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 2 filtered out; finished in 163.09s` — one live Anthropic Messages turn produced a valid normalized terminal frame with persisted, replayable state and no credential leak, per the suite's stable-contract assertions. Env gates: `SUCCESSOR_LIVE_PROVIDER_SMOKE` + `ANTHROPIC_API_KEY`. Default runs continue to skip it (verified: 2 passed, 1 ignored without the gate). Gate 5 criterion 4 satisfied; verifier task 251 had classed it awaits-live-smoke with wiring verified.
- Residuals routed:
