...
If completed — FULL RPC STAGE (task 226-C8FullRpcPreExecutionDissent, verdict PROCEED-WITH-CONDITIONS, checkout-proof `0f7812413`; full binding rulings in `agent://226-C8FullRpcPreExecutionDissent`):
- Dissent concern: the executor could invent the HTTP substrate or local route contract mid-implementation, add a premature daemon binary crossing Wave D authority, drift resume into a local session store, open credential leak paths through HTTP error surfaces, or reserialize frames into a second SSE schema.
- Response: platform precedent (axum 0.8 in B1) and contract §5/§9/§11/§13 pin the safe shape; C1/C2/C3/C7 accepted surfaces verified sufficient for wiring (C2 `KernelFrameStream::subscribe` + `render_kernel_frame_sse`, C1 full /v0 client incl. snapshot, C3 stateless re-resolution, C7 `TurnAttempt`/`TurnInput.assembly_query`); any missing seam is a narrow reopen of the owning lane, never a C8 workaround.
- Outcome: PROCEED with orchestrator rulings (full text in agent://226): (1) axum 0.8 as the routing substrate, disclosed Cargo additions limited to `axum`, `tokio-stream` (sync, for broadcast-to-body streams), and dev-only `tower` (util) if oneshot route tests need it; hand-rolled HTTP stacks, storage deps, and provider SDKs prohibited; (2) kernel stays LIB-ONLY — C8 exposes `build_router(...)`/`serve(...)`; any `[[bin]]`/daemon/CLI lifecycle belongs to Wave D; (3) minimal route surface per contract §9/§11/§13 with C8-owned local request/response wrappers embedding protocol DTOs by reference — no duplicate RawEvent/KernelFrame/platform/provider DTOs, no stable-contract drift beyond the ruled set; (4) test-file grant: C8-owned `crates/successor-kernel/tests/slice0_kernel_rpc.rs` as disclosed ownership expansion; (5) resume = session_id + platform snapshot/events/artifacts via C1 + C3 provider re-resolution with FRESH local state — local caches, session files, and provider-replay reconstruction are forbidden patterns; (6) redaction: no provider-secret inspection route; route errors/traces/logs must never format raw env/config/header values (C3 redacted types remain the only credential carriers); (7) SSE routes stream C2-rendered `event: kernel_frame` records verbatim — any second JSON/event schema is a violation; (8) replacing the C8-owned `http`/`routes`/`api` stubs and finalizing `lib.rs` is in-scope; other lanes' module declarations untouchable.
