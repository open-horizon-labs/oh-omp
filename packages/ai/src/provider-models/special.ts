...
// RunPod endpoints are deployment-specific single-model appliances: the bundled
// qwen/qwen3.8-27b entry plus a local providers.runpod.baseUrl override is the whole
// product. Live GET <baseUrl>/models is deliberately never installed (#114) — a cold
// worker takes minutes and bills GPU time to learn an id we already ship, and
// --list-models awaits refresh("online") on the same path.
export function runpodModelManagerOptions(): ModelManagerOptions<"openai-completions"> {
	return {
		providerId: "runpod",
	};
}
