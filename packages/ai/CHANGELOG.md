...
- Added opt-in proactive OAuth refresh: `AuthStorage.refreshExpiring({ provider?, expiringWithinMs? })` forces a token-endpoint call for credentials expiring within a window (default ~20% of the token TTL) and persists only when the credential actually changed. Failures never disable — the reactive path still owns that decision. Rotated credentials record `obtainedAt` to make the TTL-based default window exact ([#102](https://github.com/open-horizon-labs/oh-omp/issues/102))
- Added a crash-safe per-credential refresh lease in `agent.db` so concurrent processes sharing one store produce a single token-endpoint call and waiters re-read the rotated credential ([#103](https://github.com/open-horizon-labs/oh-omp/issues/103))
- Re-added a first-party `deepseek` provider (`deepseek-v4-flash`, `deepseek-v4-pro`) with authenticated model discovery against `<base>/v1/models`, current published pricing, and `DEEPSEEK_API_KEY`. Ships with the fork-point `reasoning: true` / effort-based thinking / `reasoningContentField` + `requiresReasoningContentForToolCalls` compat block — the known-working upstream configuration, not yet live-verified against the real API for lack of a credential. See [#107](https://github.com/open-horizon-labs/oh-omp/issues/107).

## [0.15.0] - 2026-08-22

### Added

- Added a first-party `runpod` provider with a bundled `qwen/qwen3.8-27b` template (131k context / 32k output, BF16 on vLLM). RunPod endpoints are deployment-specific, so the catalog entry carries no base URL: a `providers.runpod.baseUrl` override in `models.yml` plus `RUNPOD_API_KEY` points the model at `https://api.runpod.ai/v2/<endpoint>/openai/v1`. The provider installs no dynamic discovery: the bundled entry is the source of truth and no refresh path (including `--list-models`) requests `<base>/models` — a cold worker takes minutes and bills GPU time to learn an id we already ship. Qwen `qwen-chat-template` thinking now also passes `reasoning_effort` inside `chat_template_kwargs` — Qwen3.8's chat template accepts `low`/`medium`/`xhigh` only, so the bundled entry maps `minimal→low` and `high→xhigh`. Verified live: chat, reasoning (`message.reasoning`), structured tool calls, and tool-result replay all return 200. See [#112](https://github.com/open-horizon-labs/oh-omp/issues/112) and [#114](https://github.com/open-horizon-labs/oh-omp/issues/114).
