...
- Added a first-party `runpod` provider with a bundled `qwen/qwen3.8-27b` template (131k context / 32k output, BF16 on vLLM). RunPod endpoints are deployment-specific, so the catalog entry carries no base URL: a `providers.runpod.baseUrl` override in `models.yml` plus `RUNPOD_API_KEY` points the model at `https://api.runpod.ai/v2/<endpoint>/openai/v1`. Authenticated discovery (`GET <base>/models`) maps only the verified Qwen id (case-insensitive); unknown served ids are dropped rather than given guessed metadata. Qwen `qwen-chat-template` thinking now also passes `reasoning_effort` inside `chat_template_kwargs` — Qwen3.8's chat template accepts `low`/`medium`/`xhigh` only, so the bundled entry maps `minimal→low` and `high→xhigh`. Verified live: chat, reasoning (`message.reasoning`), structured tool calls, and tool-result replay all return 200. See [#112](https://github.com/open-horizon-labs/oh-omp/issues/112).
