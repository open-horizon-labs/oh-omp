...
- Re-added a first-party `deepseek` provider (`deepseek-v4-flash`, `deepseek-v4-pro`) with authenticated model discovery against `<base>/v1/models`, current published pricing, and `DEEPSEEK_API_KEY`. Ships with reasoning/thinking disabled by default: the `reasoningContentField`/`requiresReasoningContentForToolCalls` compat path this would otherwise exercise has no live coverage against DeepSeek in this fork, and no credential was available to run the required end-to-end tool-call verification. See [#107](https://github.com/open-horizon-labs/oh-omp/issues/107).
